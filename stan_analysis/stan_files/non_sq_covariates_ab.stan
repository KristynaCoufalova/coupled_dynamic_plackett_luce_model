...
model {
  ...
  for (n in 1:N_marathon) {
    ...
    for (i in 1:n_competitors_marathon[n]) {
      ...
      if (t >= first_appearance[m]) { // Apply age effect only after the first appearance
        real age_effect = beta_age * age_marathon[n, i];
  ...
  for (n in 1:N_half_marathon) {
    ...
    for (i in 1:n_competitors_half_marathon[n]) {
      ...
      if (t >= first_appearance[m]) { // Apply age effect only after the first appearance
        real age_effect = gamma_age * age_half_marathon[n, i];
        abilities[m] += age_effect;
      }
    }
