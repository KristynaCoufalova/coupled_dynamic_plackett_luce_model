...
parameters {
  ...
  real<lower=0, upper=1> c;  // Coupling parameter
  real<lower=0, upper=1> d;  // Coupling parameter
...
transformed parameters {
  ...
  for (m in 1:M) {
    ...
    for (next_t in (t+1):T) {
      theta[next_t, m] = a * theta[next_t-1, m] + b * eta[next_t-1, m] + 
                         dot_product(Q_M[m], theta_raw[next_t]) * sqrt((1-a^2-b^2)*sigma^2);
      eta[next_t, m] = c * eta[next_t-1, m] + d * theta[next_t-1, m] + 
                       dot_product(Q_M[m], eta_raw[next_t]) * sqrt((1-c^2-d^2)*sigma^2);
    }
  }
}
...
model {
  ...
  sigma ~ gamma(2, 2);
  a ~ uniform(0, 1);
  b ~ uniform(0, sqrt(1-a^2));  
  c ~ uniform(0, 1);
  d ~ uniform(0, sqrt(1-c^2)); 
