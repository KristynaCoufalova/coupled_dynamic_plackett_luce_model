...
fit_abcd<- readRDS("fit_abcd.rds")


#### Divergences check ####

rstan::check_hmc_diagnostics(fit_abcd)
sampler_params <- rstan::get_sampler_params(fit_abcd, inc_warmup = FALSE)
...
plot_theta_eta_evolution(fit_abcd, competitor_id = 7)
plot_theta_eta_evolution(fit_abcd, competitor_id = 12)
plot_theta_eta_evolution(fit_abcd, competitor_id = 22)
plot_theta_eta_evolution(fit_abcd, competitor_id = 23)
plot_theta_eta_evolution(fit_abcd, competitor_id = 55)       
plot_theta_eta_evolution(fit_abcd, competitor_id = 71)       
...
params_to_check <-c("a", "b","c", "d", "sigma", "theta[23,7]", "eta[23,7]", "theta[26,7]", "eta[26,7]","theta[20,55]", "eta[20,12]", "theta[25,55]", "eta[25,55]", "theta[22,71]")

# Trace plots
traceplot <- mcmc_trace(fit_abcd, pars = params_to_check)
print(traceplot)
#ggsave("traceplot.png", traceplot, width = 12, height = 8)

# Autocorrelation plots
autocorr <- mcmc_acf(fit_abcd, pars = params_to_check)
print(autocorr)
...
lags_below_threshold <- find_lag_below_threshold(fit_abcd, params_to_check, threshold = 0.1)
...
fit_summary <- rstan::summary(fit_abcd)$summary
...
# For marathon
y_rep_marathon <- as.matrix(fit_abcd, pars = "y_rep_marathon")
...
# For half-marathon
y_rep_half_marathon <- as.matrix(fit_abcd, pars = "y_rep_half_marathon")
...
nuts_params <- rstan::get_sampler_params(fit_abcd, inc_warmup = FALSE)
...
# 6. Density plots
mcmc_dens_overlay(fit_abcd, pars = params_to_check) +
  ggtitle("Density Plots for Selected Parameters")



#### Exploration of a and b and c and d ####

# Extract posterior samples for a, b, c, and d
posterior_samples <- rstan::extract(fit_abcd, pars = c("a", "b", "c", "d"))
...
pairs(posterior_df, 
      main = "Joint Posterior Distributions of a, b, c, and d",
      pch = 20,
      col = adjustcolor("blue", alpha.f = 0.1),
      lower.panel = NULL)  # This removes redundant plots below the diagonal

# Calculate posterior correlations
cor_matrix <- cor(posterior_df)
print(cor_matrix)


# Posterior summary of a, b, c, and d
params <- c("a", "b", "c", "d")
for (param in params) {
  cat(paste("\nSummary for parameter", param, ":\n"))
  samples <- posterior_df[[param]]
  cat("Mean:", mean(samples), "\n")
  cat("Median:", median(samples), "\n")
  quantiles <- quantile(samples, c(0.05, 0.25, 0.5, 0.75, 0.95))
  print(quantiles)
}

# Extract posterior samples for theta and eta
theta_samples <- rstan::extract(fit_abcd)$theta
eta_samples <- rstan::extract(fit_abcd)$eta
...
# Make sure 'data' contains 'name' and 'competitor_id' columns
theta_df <- theta_df %>%
